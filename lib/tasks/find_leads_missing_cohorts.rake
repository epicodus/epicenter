...
task :find_leads_missing_cohorts => [:environment] do
  ...
  File.open(filename, 'w') do |file|
    ...
    # ********************* #
    fulltime_students_who_should_have_starting_cohort_assigned = Student.with_deleted.select { |s| s.courses_with_withdrawn.fulltime_courses.any? && s.courses_with_withdrawn.fulltime_courses.first.start_date > Date.parse('2017-01-01') }
