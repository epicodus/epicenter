...
task :tmp_update_cohorts3 => [:environment] do
  ...
  #   student.crm_lead.update({ Rails.application.config.x.crm_fields['COHORT_STARTING'] => student.starting_cohort.description })
  # end
  # Student.where.not(cohort: nil).each do |student|
  #   student.crm_lead.update({ Rails.application.config.x.crm_fields['COHORT_CURRENT'] => student.cohort.description })
  # end
