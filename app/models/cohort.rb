class Cohort < ApplicationRecord
  ...
  def find_or_create_courses
    if track.description == 'Part-Time Intro to Programming'
      ...
      self.courses << Course.create({ language: track.languages.find_by(level: 4), start_date: skip_holidays(self.courses.last.end_date.next_week), office: office, track: track, admin: admin, start_time: '8:00 AM', end_time: '5:00 PM', active: false })
    end
  end
