...
def create_attendance_record_in_course(course, status)
  ...
  travel_to date do
    course_attendance_record = FactoryBot.create(:attendance_record, student: course.students.first, pairings_attributes: [pair_id: pair.id], tardy: status == 'tardy', left_early: status == 'left_early')
  end
end
