feature "remote attendance" do  
  context 'non-online student' do
    let(:portland_student) { FactoryBot.create(:portland_student, :with_all_documents_signed) }

    before do
      allow(IpLocation).to receive(:is_local?).and_return(false)
      login_as(portland_student, scope: :student)
    end

    describe 'sign in' do
      it 'does not show attendance sign in link in attendance box' do
        travel_to portland_student.course.start_date.beginning_of_day + 8.hours do
          visit root_path
          expect(page).to_not have_css '#attendance-sign-in-link'
        end
      end
    end

    describe 'sign out' do
      it 'does not show attendance sign out link in attendance box' do
        travel_to portland_student.course.start_date.beginning_of_day + 8.hours do
          FactoryBot.create(:attendance_record, student: portland_student)
          visit root_path
          expect(page).to_not have_css '#attendance-sign-out-link'
        end
      end
    end
  end

  context 'online student' do
    let(:student) { FactoryBot.create(:online_student, :with_all_documents_signed) }
    ...
    describe 'sign in', :js do
      let!(:pair) { FactoryBot.create(:online_student, :with_all_documents_signed, courses: [student.course]) }

      it 'shows attendance sign in link to online student' do
        travel_to student.course.start_date.beginning_of_day + 8.hours do
          visit root_path
          expect(page).to have_css '#attendance-sign-in-link'
        end            
      end
    ...
    describe 'changing pair', :js do
      let!(:pair) { FactoryBot.create(:online_student, :with_all_documents_signed, courses: [student.course], password: 'password2', password_confirmation: 'password2') }
    ...
    describe 'shows list of nonreciprocated pairs' do
      let(:other_student) { FactoryBot.create(:online_student, :with_course) }
    ...
    describe 'signing out' do
      ...
      it 'allows signing out of attendance from navbar' do
        travel_to student.course.start_date.beginning_of_day + 8.hours do
          ...
          expect(page).to have_css('#attendance-sign-out-button')
        end
      end
      ...
      context 'with a pair' do
        let!(:pair) { FactoryBot.create(:online_student, courses: [student.course]) }

        it 'directs to pair feedback' do
          travel_to student.course.start_date.beginning_of_day + 8.hours do
            FactoryBot.create(:attendance_record, student: student, pairings_attributes: [pair_id: pair.id])
            visit root_path
            click_link 'attendance-sign-out-link'
            expect(current_path).to eq pair_feedback_path
            expect(page).to have_content 'Pair feedback'
          end
        end

        it 'directs to sign out page after pair feedback' do
          travel_to student.course.start_date.beginning_of_day + 8.hours do
            FactoryBot.create(:attendance_record, student: student, pairings_attributes: [pair_id: pair.id])
            visit sign_out_path
