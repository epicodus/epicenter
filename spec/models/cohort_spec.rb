describe Cohort do
  ...
  describe 'creating an online cohort' do
    let(:office) { FactoryBot.create(:online_office) }
    let(:track) { FactoryBot.create(:online_track) }
    let(:admin) { FactoryBot.create(:admin) }

    it 'creates an online cohort and course' do
      cohort = Cohort.create(track: track, admin: admin, office: office, start_date: Date.parse('2017-03-13'))
      expect(cohort.description).to eq "PT: 2017-03 #{office.short_name} #{track.description} (Mar 13 - Jun 22)"
      expect(cohort.office).to eq office
      expect(cohort.track).to eq track
      expect(cohort.admin).to eq admin
      expect(cohort.start_date).to eq Date.parse('2017-03-13')
