  ...
  describe 'updating close.io with demographics info', :vcr, :dont_stub_crm do
    ...
    it 'updates the record successfully' do
      ...
      expect(CrmUpdateJob).to receive(:perform_later).with(lead_id, {
        "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_GENDER']}" => demographic_info.genders.join(", "), "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_PRONOUNS']}" => demographic_info.pronouns.join(", "), "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_AFTER_GRADUATION']}" => demographic_info.after_graduation, "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_BIRTH_DATE']}" => demographic_info.birth_date, "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_EDUCATION']}" => demographic_info.education, "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_DEGREE']}" => demographic_info.cs_degree, "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_SHIRT']}" => demographic_info.shirt, "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_PREVIOUS_JOB']}" => demographic_info.job, "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_PREVIOUS_SALARY']}" => demographic_info.salary, "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_RACE']}" => demographic_info.races.join(', '), "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_VETERAN']}" => demographic_info.veteran, "custom.#{Rails.application.config.x.crm_fields['DEMOGRAPHICS_DISABILITY']}" => demographic_info.disability, "addresses"=> [{:label => "mailing", :address_1 => demographic_info.address, :city => demographic_info.city, :state => demographic_info.state, :zipcode => demographic_info.zip, :country => demographic_info.country}]
        })
    ...
    it 'sends ssn encrypted' do
      ...
      expect(CrmUpdateJob).to receive(:perform_later).with(lead_id, hash_including("custom.#{Rails.application.config.x.crm_fields['SSN']}".to_sym))
